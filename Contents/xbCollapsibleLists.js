...
function _writeList() 
{
	...
	var cellStyle2 = '';
		...
		cellStyle2 = ' style="background-color: red;"';
	...
	*/
	...
	{ 
		...
		if (this.types[i] == 'list') 
		{
			if (this.l < 1 )
			{
				str += '<td width=16 nowrap valign = middle' + cellStyle + '>';
				str += '<a target="_self" href="javascript:expand(' + this.lists[i].id + ');">';
				str += '<img border=0 src="/CImage/true.gif" ID="_img' + this.lists[i].id + '" name="_img' + this.lists[i].id + '">';
				str += '</a></td>';
			}
			else if (this.l == 1 )
			{
				str += '<td width=16><img border=0 width=16 height=1 src="/CImage/blank.gif"></td>';
				str += '<td width=16 nowrap valign=middle' + cellStyle + '>';
				str += '<a target="_self" align=left href="javascript:expand(' + this.lists[i].id + ');">';
				str += '<img border=0 src="/CImage/true.gif" ID="_img' + this.lists[i].id + '" name="_img' + this.lists[i].id + '">';
				str += '</a></td>';
			}
			else if (this.l == 2 )
		...
		str += '<TD HEIGHT="' + (this.height-3)+ '" WIDTH="' + (this.width-15-this.l*this.i) + 
			'" VALIGN="MIDDLE" ALIGN="LEFT"' + cellStyle + '>';

		// *** add link to each list heading
		if (this.types[i] == 'list')
		{
			str += '<a target="_self" align=left href="javascript:expand(' + this.lists[i].id + ');">';
		}
...
function _showList() 
{
	...
	{ 
		...
		// *** patch to provide a highlight bar for selected menu item
		
		if( navigator.family == 'nn4' )
		{
			if ( this.idNum[i] == previousSelection )
			{
				bg = normalBgColor;
			}
		}			
		if ( this.idNum[i] == currentSelection )
		{
			bg = highlightColor;
		}
	...
	{ 
		...
		if (this.types[i] == 'list') 
		{
			if (this.lists[i].visible) 
			{
				...
				setImage(this.lists[i], item, '/CImage/true.gif');
			} 
			else 
				setImage(this.lists[i], item, '/CImage/false.gif');
...
function addItem(str, bgColor, item) 
{
	...
	this.idNum[this.idNum.length] = _item_id;
	++_item_id;
}
